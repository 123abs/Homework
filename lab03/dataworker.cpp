...
/**
 * @brief 设置请求城市的setter函数
 * @param city 查询的城市
 *
 */
void dataWorker::setRequestCity(QString city)
{
    _requestCity = city;
}
...
void dataWorker::doRequest()
{
    ...
    QString fName;
        if(_requestType)
        fName = QString("%1/weather_%2-%3.txt").arg(dataPath,_requestCity,_requestDate);
        else fName = QString("%1/aqi_%2-%3.txt").arg(dataPath,_requestCity,_requestDate);
    //    qDebug()<<fName;
...
{    
    QString r;
       if(_requestType)
           ...
           r =
                   QString("http://www.tianqihoubao.com/aqi/%1-%2.html").arg(_requestCity).arg(_requestDate);
       qDebug()<<r;
       return r;
}
...
void dataWorker::parseHTML(const QString sourceText)
{
    ...
    while (!reader.atEnd()) {
        ...
        if (reader.isStartElement()) {
            if (reader.name() == "ul"||reader.name()=="tr"){         // 查找Html标签：ul
    ...
    for (QString s : dataList){
        ...
        dataLow.append(dataList.at(2).toDouble());
           ...
           for (QString s : dataList){
               ...
               QDateTime momentInTime = QDateTime::fromString(dataList.at(0),"yyyy-MM-dd");
               dataDate.append(momentInTime);
               dataHigh.append(dataList.at(4).toDouble());//pm2.5
               dataLow.append(dataList.at(5).toDouble());//pm10
     }
...
void dataWorker::exportDataToFile(const QString dataText)
{
    ...
    QString fName;
       if(_requestType) fName = QString("%1/weather_%2-%3.txt").arg(dataPath,_requestCity,_requestDate);
       else    fName = QString("%1/aqi_%2-%3.txt").arg(dataPath,_requestCity,_requestDate);
...
void dataWorker::httpsFinished(QNetworkReply *reply)
{
    ...
    int begin,end;
        if(_requestType){
            ...
            begin = html.indexOf("<table width=\"620px\" border=\"0\" class=\"b\" cellpadding=\"1\" cellspacing=\"1\">");
