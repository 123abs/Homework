...
void mainWidget::initComboMonth()
{
    ...
    QDate date = QDate::currentDate();
       qint64 current_year=date.year();
       qint64 current_month=date.month();
       for(int i=1;i<=20;i++){
           ...
           month<<QString("%1-%2").arg(current_year).arg(current_month-i,2,10,QChar('0'));
           }
    ui->comboMonth->clear();
...
void mainWidget::on_btnStart_clicked()
{
    ...
    QString chartTitle = "";
       if(ui->comboMonth->count()>0){
           ...
           chartTitle.append("月 ");
           chartTitle.append(ui->comboBox->currentText());
           if(searchType)chartTitle.append("气温");
           else chartTitle.append("AQI");
       }else{
           chartTitle = ui->comboBox->currentText();
           if(searchType)chartTitle.append("气温");
           else chartTitle.append("AQI");
       }
       ...
       // 设置dataWorker对象的请求类型
       worker->_requestType=(ui->temRadioButton->isChecked());

       // 设置dataWorker对象的请求年月
       worker->setRequestDate(ui->comboMonth->currentText());

       // 设置dataWorker对象的请求城市
       worker->setRequestCity(mainWidget::transCityName(ui->comboBox->currentText()));
...
void mainWidget::updateDataChart(QList<QDateTime> date, QList<qreal> tempHigh, QList<qreal> tempLow)
{
    ...
    // 添加第二条数据曲线
        if(searchType)
                ...
                addLineSeries(chart,"PM10",Qt::blue,2);
    ...
    //获取序列最值
        qreal _min=1000,_max=-1;
        for(qreal i : tempHigh){
                _min=qMin(_min,i);
                _max=qMax(_max,i);
        }
        for(qreal i : tempLow){
    ...
    // 设置坐标轴
        QDateTimeAxis *mAxisX = qobject_cast<QDateTimeAxis*>(chart->axisX());
            ...
            mAxisY->setRange(_min-10,_max+10);
...
QString mainWidget::transCityName(QString originCity){
    QString changedCity;
    if(originCity=="南京")changedCity ="nanjing";
    if(originCity=="上海")changedCity ="shanghai";
    if(originCity=="北京")changedCity ="beijing";
    if(originCity=="广州")changedCity ="guangzhou";
    if(originCity=="福州")changedCity ="fuzhou";
      return changedCity;
}

void mainWidget::on_temRadioButton_clicked(bool checked)
{
    searchType=true;
}

void mainWidget::on_aqiRadioButton_clicked(bool checked)
{
    searchType=false;
}
