...
int main(int argc, char *argv[])
{
    ...
    quint32  Hex32,Hex32_new;   //已知32位数，重组后32位数
    quint16  H16,L16;           //32位数拆分为的高16位和低16位
    quint16  MAX1,MIN1;         //高16位数的较大值，低16位数较小值
    quint8   HHI,HLO,LHI,LLO;   //32位数拆分为高八位，次高八位，次低八位，低八位变量
    QList<quint8> values;
    Hex32=0x45678932;
    ...
    //10进制转换16进制
    QString str = QString("0x%1").arg(MAX1,2,16);     //MAX1转换为16进制
    QString str1 = QString("0x%1").arg(MIN1,2,16);    //MIN1转换为16进制
    QString str2 = QString("0x%1").arg(Hex32_new,8,16);

    qDebug()<<"最高八位和次高八位最大值"<< str << MAX1;     //分别输出MAX1的16进制和10进制所表示数
    qDebug()<<"最低八位和次低八位最小值"<< str1<< MIN1;     //分别输出MIN1的16进制和10进制所表示数
    qDebug()<<"重新组合的32位数"<< str2 << Hex32_new;      //分别输出重新组合后32位数的16进制和10进制所表示数
    qDebug()<<"排序后"<<values;                           //输出提取出的八位数排序
